...
> **Names, not versions.** **Classic planr** means the commands planr ships
...
>
> This captures the reasoning and decisions from the `next` brainstorm so they
...
Classic planr hard-wires two things that turn out to be project-specific:
...
- **Derive, don't store.** All groupings/rollups are computed by scanning at
  read time (as classic planr already does for the board). Any index is a *disposable,
...
- **Compatibility is outcome-equivalence, not byte-identity** (round-2). The
  default schema runs existing classic planr backlogs and produces the same *gate/merge
  outcomes* — but it is not byte-for-byte classic planr (verdict-as-state alone guarantees
...
- **The unit is a declared *cut* across the tree, not necessarily its leaf.**
  ...
  per task. This decouples two things classic planr conflated by accident: the execution
...
- `parent` on the **child** → the child owns its membership; abandon/archive the
  ...
  whoever still points at it. This is classic planr's "no agent ever edits a parent to
...
Every state change flows through a verb — including the ones classic planr did as **manual
file edits** (the worker hand-setting `status: review`, the reviewer hand-writing
the verdict). In `next` those become `submit` / `approve` / `request-changes`, which
...
- **`merge`** — integrate the new commit into `home`. On conflict, **abort and
  print rebase guidance** (classic planr behaviour); the target is left untouched, and the
...
`plan: <verb> <slug>`, asserting no state — classic planr's `plan: claim <slug>
...
*language*, not the binary — a planr release does not move the URL unless the
...
(This corrects an earlier draft that used `terminal` for both. Classic planr gated
...
declared in a dedicated schema key (working name **`templates`**, echoing classic planr's
...
serialize. The flat-file rewrite preserves classic planr's coordination guarantee and
tightens it — two fewer locks than classic planr, and one fewer than the round-2 design.
...
scanning `tickets/` (as classic planr's board already does); cold tickets by walking
...
"Byte-for-byte classic planr" is impossible by construction: classic planr has no `approved` state and
...
content-parse the `next` `require` vocabulary deliberately *cannot* do (§3.6). Since
verdict-as-state (R3) is core to `next`, reproducing classic planr's file/history exactly is a
non-goal. So there is **one recommended preset**, and compatibility means the
same *gate/merge outcomes* on a migrated backlog:

- **default (recommended)** — `kinds: [epic, story, task]`, the `next` verbs and
  derived lifecycle. Relative to classic planr it carries two intentional fixes: container
...
- A more conservative variant (container-close on `done`, review left as manual
  edits) is expressible as a schema, but is *not* byte-for-byte classic planr either — the
  verdict mechanism still differs. A project that genuinely needs classic planr's exact
...
same inputs) between classic planr and the `next` default on migrated backlogs — not byte
...
`status`, `id`, `created` and `updated`, but a classic planr ticket carries no `Planr-Verb`
...
12. **Migration seed events** — the exact trailer shape `planr migrate` writes to
    seed a classic planr ticket's event chain (§7), and whether a seed is one commit per
...
Classic planr -> `next` data shape; workflow version to workflow version is unspecified.
...
doc against the classic planr source. Assessment column is *this author's* triage, not the
...
- **R8 — "Byte-for-byte classic planr" was false. ✅ RESOLVED (§7)** — *superseded by B2
  (round 2), which went further:* "byte-for-byte" is dropped entirely (classic planr's
...
directory hack; needs-vs-decomposition asymmetry (§3.6, confirmed against classic planr
...
- **B2 — "byte-for-byte classic planr" was self-contradictory and unbuildable. ✅ FIXED
  (§2/§7).** §2 claimed the default is byte-for-byte while §7 said it's a
  superset; and `classic-strict` couldn't reproduce classic planr's free-text `verdict:` parse
  ...
  preset; classic planr's exact section-parse is only reachable via a `do` hook.
...
- **Request A — container integration branches: deferred**, but its *contract*
  ...
  parent chain, with trunk as the base case rather than a constant (§3.4). Classic planr
  left this variable and `next` had re-fixed it; restoring it makes A a later schema
  ...
  not conflict.
- **Request C — no classic planr changes**: nothing asked, nothing done.

### Two corrections to the design as written

- **Worktree location.** classic planr already defaults to `.plan/worktrees/` and already
...
runner over the five-verb loop for a single `task` kind. Classic planr was untouched
...
- **A latent classic planr bug the new ordering exposes.** `git.rs`'s `worktree_add`
  omits the branch argument when the branch already exists, so
  `git worktree add <path>` invents a branch named after the path. Classic planr never
  reaches that path because it creates branch and worktree together; the `next`
...
The two stay separate for now, to keep the upgrade path from classic planr open. Recording
