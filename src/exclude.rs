//! Local ignore rules (`.git/info/exclude`) and the path arithmetic they
//! rest on.
//!
//! A worktree that lands inside a working tree has to be hidden from git, or
//! `git add` stages it as a gitlink. The rule that hides it is anchored to a
//! path, so working out the right path -- resolving it, finding which
//! worktree contains it, and expressing it as a gitignore pattern -- is the
//! larger half of the job and lives here too.
//!
//! `.git/info/exclude` is the user's file. planr owns the lines inside its
...
mod tests {
    use super::*;
    use crate::git::tests::with_temp_repo;
    use std::fs;
